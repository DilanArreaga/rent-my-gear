...
const MOCK_INVENTORY: GearItem[] = [
  {
    ...
    description: "Camara mirrorless profesional 45MP",
  ...
  },
  {
    ...
    description: "Tienda para 4 personas resistente al agua",
    specs: { capacity: "4 personas", weight: "3.5kg" },
    dailyRate: 200,
    imageURL: "",
  },
  {
...
export async function getGearByCategory(categoryId: string): Promise<GearItem[]> {
  if (!isValidCategory(categoryId)) throw new Error(`Invalid category: ${categoryId}`);
...
> {
  ...
  for (const item of items) {
    ...
    if (item.imageURL) stats[item.category].withImages++;
    else stats[item.category].withoutImages++;
  }
  return stats;
}
