...
export function getInsuranceRate(category: string): number {
  return isHighRiskCategory(category) ? INSURANCE_RATES.HIGH_RISK : INSURANCE_RATES.STANDARD;
}
...
export function calculateInsurance(category: string, dailyRate: number, days: number): number {
  const rate = isHighRiskCategory(category) ? INSURANCE_RATES.HIGH_RISK : INSURANCE_RATES.STANDARD;
